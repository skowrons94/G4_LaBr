...
G4VPhysicalVolume* LaBr1Detector::Construct()
{
	...
	G4double ReflectorStart = 3.3245*mm;
	G4double ScintStart = 6.3245 *mm;
	...
	G4Polycone* solidCrystal = new G4Polycone("LaBr1Crystal",StartPhi,DeltaPhi,crystalNZPlanes,
	...
	G4Polycone* solidPMT = new G4Polycone("LaBr1PMT",StartPhi,DeltaPhi,pmtNZPlanes,pmtZPlanes,pmtRInner,pmtROuter);

	//G4Hype* solidPMT = new G4Hype("LaBr1PMT", 0. * cm, PMTRadiusLarge, 0., 0., 2 * cm);
	...
	G4double ReflectorRadiusOuter = 4.433 *cm;
	...
	G4Polycone* solidReflector = new G4Polycone("LaBr1Reflector",StartPhi,DeltaPhi,reflectorNZPlanes,
	...
	G4Polycone* solidHousing = new G4Polycone("LaBr1Housing",StartPhi,DeltaPhi,housingNZPlanes,
	...
	G4VisAttributes* red= new G4VisAttributes(G4Colour(1.5,0.0,1.0));
	...
	G4VisAttributes* grey= new G4VisAttributes(G4Colour(0.6,0.6,0.6));
...
void LaBr1Detector::ConstructSDandField()
{
    ...
    auto *det = new G4MultiFunctionalDetector("LaBr1Crystal");
    G4SDManager::GetSDMpointer()->AddNewDetector(det);

    auto *psEdep = new G4PSEnergyDeposit("Edep",1);
    det->RegisterPrimitive(psEdep);

    SetSensitiveDetector( "LaBr1Crystal", det);
}

void LaBr1Detector::SetupOutput()
{
    auto *am = G4AnalysisManager::Instance();

    fTupleID = am->CreateNtuple("EdepLaBr1", "Energy Deposition in LaBr1 detector");

    G4cout << "LaBr1Detector::SetupOutput - " << fTupleID << G4endl;

    am->CreateNtupleDColumn("LaBr1");
 
    am->FinishNtuple();
}

void LaBr1Detector::FillOutput(const G4Event *event)
{
    // Get hit collection ID
    if (fHCID == -1 )
    {
        fHCID = G4SDManager::GetSDMpointer()->GetCollectionID("LaBr1Crystal/Edep");
    }
